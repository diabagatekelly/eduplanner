  ...
  return (
    <>
      {listData?.length ?
        <ul>
          {listData?.map((dataPt) => (
            <li className="flex justify-between" key={dataPt}>
              <p className="hover:cursor-pointer" onClick={() => fetchListItem(dataPt)}>{dataPt}</p>
              <span onClick={() => deleteListItem(dataPt)}>
        ...
        <p>You have no active students.</p>
      }
