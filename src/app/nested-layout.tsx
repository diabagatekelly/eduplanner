...
export default function NestedLayout({ children, isTeacher }) {
  ...
  return (
    <>
      <button onClick={() => toggleDrawer(!openDrawer)} data-drawer-target="default-sidebar" data-drawer-toggle="default-sidebar" aria-controls="default-sidebar" type="button" 
      ...
      <aside id="default-sidebar" className={openDrawer ? "fixed top-16 left-0 z-0 w-64 h-screen transition-transform sm:translate-x-0 -translate-x-full" : "fixed top-25 left-0 z-0 w-64 h-screen transition-transform sm:translate-x-0"} aria-label="Sidebar">
              ...
              <>
                <li>
                  <a href={pathname} className={"flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" + classNames(pathname === `/${user.username}/students` ? " bg-gray-800 text-white hover:bg-gray-700" : "")}>
                    ...
                    <span className="flex-1 ml-3 whitespace-nowrap">Activity: {activityPath.split('-').join(' ')}</span>
                  </a>
                </li>
                <li>
