...
export function CardsList({isMain, userDetails, getBorderColor, ...childArgs}) {
  ...
  function advanceCard(card: ICard) {
    getPopupItem({card, action: 'advance'})
    getPopupUserDetails(userDetails)
    setModalType('manageCard')
    setShowModal(true);
  }
  ...
  return (
    <>
      {hash === '' && 
        <>
          <h3 className="mb-2 text-2xl font-bold">Cards of the Day</h3>
          <ul>
            {!cardsOfTheDay.length ? <p>You have no cards to review today.</p> :
            cardsOfTheDay?.length &&            
            cardsOfTheDay?.map((card) => (
              <li style={{ borderColor: getBorderColor(card) }} className="flex justify-between border-4 mb-3" key={card.cardId}>
                <p>{formatCardName(card.cardId)}</p>
                <div className="flex">
                  <span className="hover:cursor-pointer mx-2" onClick={() => showCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                      <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                    </svg>
                  </span>
                  <span className={card.completionStatus !== CompletionStatus.COMPLETED ? "hover:cursor-pointer mx-2" : "hover:cursor-pointer mx-2 hidden"} onClick={() => editCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>
                  </span>
                  <span className={!isMain || isMain && userDetails?.accountType === 'teacher' ? "hover:cursor-pointer mx-2": "hidden"} onClick={() => deleteCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
            ...
            allActiveCards?.map((card) => (
              <li style={{ borderColor: getBorderColor(card) }} className="flex justify-between border-4 mb-3" key={card.cardId}>
                ...
                <div className="flex">
                  ...
                  <span className={card.completionStatus !== CompletionStatus.COMPLETED ? "hover:cursor-pointer mx-2" : "hover:cursor-pointer mx-2 hidden"} onClick={() => advanceCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>
                  </span>
        ...
        <>
          ...
          <ul>
            ...
            allInactiveCards?.map((card) => (
              <li className="flex justify-between border-4 mb-3" key={card.cardId}>
                <p>{formatCardName(card.cardId)}</p>
                <div className="flex">
                  <span className="hover:cursor-pointer mx-2" onClick={() => showCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                  ...
                  <span className={!isMain || isMain && userDetails?.accountType === 'teacher' ? "hover:cursor-pointer mx-2": "hidden"} onClick={() => activateCard(card)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
