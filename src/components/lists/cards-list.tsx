...
import { removeUserCard } from "@/store/actions/userActions";
    ...
    <>
      <AddCard {...{ activity: childArgs.activity, mode: mode, resetMode: resetMode, cardToEdit: editList }} />
      {
        <ul>
          {cardsList?.map((card) => (
           cardsList?.length &&            
            <li style={{ borderColor: getBorderColor(card) }} className="flex justify-between border-4" key={card.id}>
              <p className="hover:cursor-pointer" onClick={() => fetchCard(card.front)}>{card.front} {card.back} {card.stage}</p>
