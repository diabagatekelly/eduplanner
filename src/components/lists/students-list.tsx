...
export default function StudentsList({userDetails, getBorderColor}: {userDetails: IUser, getBorderColor: any}) {
  ...
  const [studentIdsList, getStudentIdsList] = useState<string[]>([])
        ...
        <ul>
          {studentIdsList?.map((userId) => (
            <li data-testid="students-list" style={{ borderColor: getBorderColor('students') }} className="flex justify-between border-4" key={userId}>
              <p data-testid="students-email" className="hover:cursor-pointer" onClick={() => fetchStudent(userId)}>{studentNames.length && studentNames[studentIdsList.indexOf(userId)] ? studentNames[studentIdsList.indexOf(userId)] : atob(userId)}</p>
