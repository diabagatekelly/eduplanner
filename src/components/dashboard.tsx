...
export default function Dashboard({ userDetails, isMain, isTeacher }: { userDetails: IUser, isMain: boolean, isTeacher: boolean }) {
  ...
  return (
    <>
      {isMain ?
        <div>
          <h3 data-testid="dashboard-header" className="component-title">Welcome {fullName}!</h3>
          {isTeacher ?
            ...
            <div className="student-dashboard-view my-5">
              <div>
                {userDetails.linkedAccountsData?.teacher ?
                  <h5 className="text-lg md:text-xl" data-testid="student-instructions">{`Your teacher's email is ${atob(userDetails?.linkedAccountsData?.teacher)}`}</h5> :
                  <h5 className="text-lg md:text-xl" data-testid="student-instructions">Ask your teacher or parent to add you and create some activities for you!</h5>
                }
              </div>
              <hr />
              <ListUi {...{ listType: 'activities', isMain, userDetails }} />
