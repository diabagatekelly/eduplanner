...
describe('User actions', () => {
  ...
  describe('Student', () => {
    it('should call EDIT reducer with the user_data updated with new linked account student id (add new student)', () => {
      ...
      expect(
        updatedUser.linkedAccountsData.students.some((tuple) => tuple[0] === mockStudent.userId)
      ).toBe(true)
    })
    ...
    it('should call EDIT reducer with the user_data updated with new additional linked account student id (add 2nd new student)', () => {
      ...
      const expectedUpdatedUser = {
        ...
        linkedAccountsData: {
          students: [
            ...
            [secondStudent.userId, secondStudent.username],
          ],
        },
      }
      const updatedUser = JSON.parse(mockSessionStorage.getItem('user_data'))
      ...
      expect(updatedUser).toMatchObject(expectedUpdatedUser)
      expect(
        updatedUser.linkedAccountsData.students.some((tuple) => tuple[0] === secondStudent.userId)
      ).toBe(true)
    })
    ...
    it('should call EDIT reducer with the user_data updated with new additional student details (save 2nd student details)', () => {
      ...
      expect(updatedUser).toMatchObject(expectedUpdatedUser)
      expect(updatedUser.students).toMatchObject({
  ...
  describe('Activity', () => {
    ...
    it('should call EDIT reducer with user_data updated with student new activity (create new activity for student)', () => {
      ...
      const reducer = createUserActivity({
      ...
      expect(JSON.stringify(updatedUser.students['mock-student'].activities[0])).toEqual(
    ...
    describe('With activity', () => {
      ...
      const teacher = {
        ...
        linkedAccountsData: { students: [student.userId] },
  ...
  describe('Cards', () => {
    describe('Create cards', () => {
      it('should call EDIT reducer with user_data updated with main user new cards (create new cards for main user)', () => {
        ...
        const reducer = createUserCard({
          username: teacher.username,
          activityName: 'Quran',
          newCards: [mockUserCard],
        })
        ...
        expect(JSON.stringify(updatedUser.activities[0].cards[0])).toEqual(
          JSON.stringify(mockUserCard)
        )
      })
      ...
      it('should call EDIT reducer with user_data updated with student user new cards (create new cards for student user)', () => {
        ...
        const currentUserStudents = currentUser.linkedAccountsData?.students?.length
        const currentUserStudentsDetails = currentUser.students
        const currentUserStudentsActivities =
          currentUserStudentsDetails[mockStudent.username].activities
        ...
        expect(
          JSON.stringify(updatedUser.students[mockStudent.username].activities[0].cards[0])
        ).toEqual(JSON.stringify(mockUserCard))
      })
    })
    ...
    describe('Edit cards', () => {
      it('should call EDIT reducer with user_data updated with main user edited cards (edit cards for main user)', () => {
        ...
        expect(JSON.stringify(userWithCards.activities[0].cards[0])).toEqual(
          JSON.stringify(mockUserCard)
        )

        const reducer = editUserCard({
          username: teacher.username,
          activityName: 'Quran',
          updatedCard: { ...mockUserCard, completionStatus: CompletionStatus.COMPLETED },
        })
        ...
        expect(updatedUser).toMatchObject(expectedUpdatedUser)
        expect(JSON.stringify(updatedUser.activities[0].cards[0])).toEqual(
          JSON.stringify({ ...mockUserCard, completionStatus: CompletionStatus.COMPLETED })
        )
      })
      ...
      it('should call EDIT reducer with user_data updated with student user edited cards (edit cards for student user)', () => {
        ...
        const studentWithUpdatedCards = {
          ...mockStudent,
          activities: [{ ...mockActivity, cards: [updatedCard] }],
        }
        ...
        expect(
          JSON.stringify(updatedUser.students[mockStudent.username].activities[0].cards[0])
        ).toEqual(JSON.stringify(updatedCard))
      })
    })
