...
describe('Breadcrumbs', () => {
  describe('Home', () => {
    it('should display "Home" breadcrumbs and navigate as expected', async () => {
      jest
        ...
        .mockImplementation(() => `root/${mockUser.username}`)
      render(<Breadcrumbs />)
  ...
  describe('Dashboard', () => {
    it('should display "Home" and "Dashboard" breadcrumbs and navigate as expected', async () => {
      jest
        .spyOn(require('next/navigation'), 'usePathname')
        .mockImplementation(() => `root/${mockUser.username}/students`)
      render(<Breadcrumbs />)
  ...
  describe('Main user activity', () => {
    it('should display "Home" and "Dashboard" and an activity in breadcrumbs', async () => {
      jest
        .spyOn(require('next/navigation'), 'usePathname')
        .mockImplementation(
          () => `root/${mockUser.username}/activities/${mockLanguageActivity.name}`
        )
      render(<Breadcrumbs />)
  ...
  describe('Student', () => {
    describe('Student activities list', () => {
      it('should display "Home" and "Dashboard" and a student name in breadcrumbs', async () => {
        jest
          .spyOn(require('next/navigation'), 'usePathname')
          .mockImplementation(() => `root/${mockUser.username}/students/Yusuf-Spencer`)
        render(<Breadcrumbs />)
    ...
    describe('Student activity', () => {
      it('should display "Home" and "Dashboard", "All student activities", and an activity in breadcrumbs', async () => {
        jest
          ...
          .mockImplementation(
            () =>
              `root/${mockUser.username}/students/Yusuf-Spencer/activities/${mockLanguageActivity.name}`
          )
        render(<Breadcrumbs />)

        const breadcrumbsHome = await screen.findByTestId('breadcrumbs-Home')
        const breadcrumbsDashboard = await screen.findByTestId('breadcrumbs-Dashboard')
        const breadcrumbsStudentActivities = await screen.findByTestId(
          "breadcrumbs-All Yusuf-Spencer's Activities"
        )
